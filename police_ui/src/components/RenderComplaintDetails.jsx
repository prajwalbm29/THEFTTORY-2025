...
const RenderComplaintDetails = ({ complaint, complaintType, status, description }) => {
    ...
    return (
        <View style={styles.detailsContainer}>
            ...
            {/* Complaint Status */}
            <ComplaintStatus status={status} description={description} />
        </View>
    );
};

const ComplaintStatus = ({ status, description }) => {
    if (!status || status.length === 0) {
        return (
            <>
                <View style={styles.statusPendingContainer}>
                    <View style={styles.statusPendingCard}>
                        <Text style={styles.statusPendingText}>No Updates Provided</Text>
                        <Text style={styles.statusPendingSubText}>Please Work on the Complaint..</Text>
                    </View>
                </View>
            </>
        );
            ...
            <View style={styles.detailsCard}>
                {status.map((item, index) => (
                    <View key={index} style={styles.statusItem}>
                        <View style={[styles.statusIndicator, styles.statusIndicatorCompleted]}>
                            <View style={styles.statusCheckmark}>
                                <Text style={styles.statusCheckmarkText}>✓</Text>
                            </View>
                        </View>
                        <View style={styles.statusContent}>
                            <Text style={[styles.statusName, styles.statusNameCompleted]}>
                                {item}
                            </Text>
                            <Text style={styles.statusDescription}>
                                {description?.[index] || 'No description provided.'}
                            </Text>
                        </View>
                    </View>
                ))}
