...
export default function Login() {
  ...
  const handleFetchDetails = async () => {
    if (aadhaarNo.length !== 12) {
      Toast.show({
        type: 'error',
        text1: 'Enter valid 12 digit Aadhaar number.',
      });
      return;
    }
    ...
    try {
      setLoading(true);
      const { data } = await axios.get(`/api/v1/auth/get-details/${aadhaarNo}`);
      if (!data?.success) {
      ...
      Toast.show({
        type: 'error',
        text1: error?.response?.data?.message || 'Aadhaar Number does not exists.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyOtp = async () => {
    try {
      setLoading(true);
      const { data } = await axios.post('/api/v1/auth/verify-otp', { otp, aadhaarNo });
      if (data?.success) {
        Toast.show({ type: 'success', text1: data?.message || 'Login successful' });
        navigation.replace('Home');
        await login(data?.token, details.name, aadhaarNo);
      } else {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.box}>
        <Text style={styles.heading}>Public Login</Text>
        ...
        {!details && (
          <TouchableOpacity
            style={[
              styles.button,
              { backgroundColor: aadhaarNo.length === 12 || loading ? '#1e90ff' : '#ccc' },
            ]}
            disabled={aadhaarNo.length !== 12 || loading}
            onPress={handleFetchDetails}
          >
            <Text style={styles.buttonText}>{loading ? 'Fetching details...' : 'Fetch Details'}</Text>
          </TouchableOpacity>
        )}
