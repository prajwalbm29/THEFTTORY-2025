...
const GoldForm = () => {
  ...
  const validateDetails = () => {
    const { aadhaarNo, weight, uniqueFeature, lostLocation, lostDescription, lostDate, invoice } = complaintDetails;

    if (aadhaarNo.trim().length !== 12) {
      showToast('error', 'Aadhaar Number is not valid.');
      return false;
    }
    if (!weight) {
      showToast('error', 'Enter Weight in grams.');
      return false;
    }
    if (!uniqueFeature) {
      showToast('error', 'Enter Gold Unique feature.');
      return false;
    }
    if (!lostLocation) {
      showToast('error', 'Enter Gold Lost Location.');
      return false;
    }
    if (!lostDescription) {
      showToast('error', 'Enter Gold Lost Description.');
      return false;
    }
    if (!lostDate) {
      showToast('error', 'Enter Gold Lost Date.');
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post('/api/v1/complaint/gold-complaint', { complaintDetails });
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Gold Theft Complaint</Text>

      {/* Laptop Information Section */}
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Gold Information</Text>

        <FormField
          label="Gold Weight [grams]*"
          placeholder="Enter Gold Weight"
          keyboardType="phone-pad"
          value={complaintDetails.weight}
          onChangeText={val => handleChange('weight', val)}
        />

        <FormField
          label="Gold Unique Feature*"
          placeholder="Enter unique feature"
          value={complaintDetails.uniqueFeature}
          onChangeText={val => handleChange('uniqueFeature', val)}
        />
      ...
      <View style={styles.section}>
        ...
        <FormField
          ...
          value={complaintDetails.witness}
          onChangeText={val => handleChange('witness', val)}
          multiline
          textArea
        />
