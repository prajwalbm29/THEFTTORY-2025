...
const LaptopForm = () => {
  ...
  const validateDetails = () => {
    const { serialNo, brand, model, lostLocation, lostDescription, lostDate, invoice, description } = complaintDetails;

    if (!serialNo.trim()) {
      showToast('error', 'Enter valid Serial Number.');
      return false;
    }
    if (!description.trim()) {
      showToast('error', 'Enter Laptop description.');
      return false;
    }
    if (!brand) {
      showToast('error', 'Enter Laptop Brand.');
      return false;
    }
    if (!model) {
      showToast('error', 'Enter Laptop Model.');
      return false;
    }
    if (!lostLocation) {
      showToast('error', 'Enter Laptop Lost Location.');
      return false;
    }
    if (!lostDescription) {
      showToast('error', 'Enter Laptop Lost Description.');
      return false;
    }
    if (!lostDate) {
      showToast('error', 'Enter Laptop Lost Date.');
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post('/api/v1/complaint/laptop-complaint', { complaintDetails });
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Laptop Theft Complaint</Text>

      {/* Laptop Information Section */}
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Laptop Information</Text>

        <FormField
          label="Serial Number*"
          placeholder="Enter Serial Number"
          value={complaintDetails.serialNo}
          onChangeText={val => handleChange('serialNo', val)}
        />
        ...
        <FormField
          label="Laptop Model*"
          placeholder="e.g., Laptop Inspiron 15"
