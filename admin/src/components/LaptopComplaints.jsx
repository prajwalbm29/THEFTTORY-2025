...
const LaptopComplaints = () => {
  const [complaints, setComplaints] = useState([])

  const fetchComplaints = async () => {
    try {
      const { data } = await axios.get('/api/v1/admin/fetch-laptop-complaints')
      if (data?.success) {
      ...
      {complaints.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {complaints.map((complaint) => (
            <Link key={complaint._id} to={`/admin/complaints/laptop/${complaint._id}`} className='hover-shadow-lg transition transform hover:scale-105'>
...
export default LaptopComplaints
