...
const Laptop = () => {
  ...
  const fetchAllocation = async () => {
    try {
      const { data } = await axios.get('/api/v1/admin/get-laptop-complaint-allocation')
      if (data?.success) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-center mb-6 text-blue-400">💻 Laptop Complaints</h1>
      <table className="w-full table-auto border border-gray-300 rounded-lg shadow-md">
        <thead>
          <tr className="bg-blue-100 text-left text-sm font-semibold text-gray-700">
            <th className="px-4 py-2 border-b border-gray-300">Serial Number</th>
        ...
        <tbody>
          {complaints.map((complaint) => (
            ...
            >
              <td className="px-4 py-2 border-b border-gray-200">{complaint.serialNo}</td>
