...
const LaptopDetails = () => {
  ...
  const getDetails = async () => {
    try {
      const { data } = await axios.get(`/api/v1/complaint/laptop-details/${id}`);
      if (data?.success) {
        setComplaint(data?.laptop);
      }
    } catch (error) {
      console.error(error);
      toast.error(error?.response?.data?.message || 'Failed to fetch laptop details.');
    }
  };
  ...
  const getStatus = async () => {
    try {
      ...
      console.error(error);
      toast.error('Failed to fetch complaint status.');
    }
  };
  ...
  const toggleVerification = async () => {
    try {
      setLoading(true);
      const { data } = await axios.patch(`/api/v1/admin/update-laptop-status/${id}`);
      if (data?.success) {
  ...
  const fetchInvoice = async () => {
    try {
      const { data } = await axios.get(`/api/v1/complaint/laptop-invoice/${id}`);
      if (data?.success) {
        setInvoice(data?.invoice);
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || 'Error in fetching invoice.');
      console.error(error);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 mb-10 p-6 bg-white rounded-xl shadow-lg space-y-4 border">
      <h2 className="text-2xl font-semibold text-center mb-4">💻 Phone Complaint Details</h2>
      <div className="space-y-2 text-gray-700">
        <p><strong>Serial Number:</strong> {complaint.serialNo}</p>
        ...
        {invoice && (<img
          src={`data:${invoice.type};base64,${invoice.base64}`}
          alt={invoice.name}
          className="mt-4 border rounded-lg max-w-full h-auto shadow-md"
        />)}
      </div>
