...
const CarComplaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const { data } = await axios.get('/api/v1/admin/fetch-car-complaints')
      if (data?.success) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-6 text-blue-700">🚗 Car Complaints</h1>
      ...
      {complaints.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {complaints.map((complaint) => (
            <Link key={complaint._id} to={`/admin/complaints/car/${complaint._id}`} className='hover-shadow-lg transition transform hover:scale-105'>
