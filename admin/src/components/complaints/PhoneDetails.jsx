...
const PhoneDetails = () => {
  ...
  const getDetails = async () => {
    try {
      const { data } = await axios.get(`/api/v1/complaint/phone-details/${id}`);
      if (data?.success) {
        setComplaint(data.phone);
      }
    } catch (error) {
      console.error(error);
      toast.error(error?.response?.data?.message || 'Failed to fetch phone details.');
    }
  };
  ...
  const toggleVerification = async () => {
    try {
      setLoading(true);
      const { data } = await axios.patch(`/api/v1/admin/update-phone-status/${id}`);
      if (data?.success) {
  ...
  const fetchInvoice = async () => {
    try {
      const { data } = await axios.get(`/api/v1/complaint/phone-invoice/${id}`);
      if (data?.success) {
