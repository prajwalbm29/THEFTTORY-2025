...
const BikeDetails = () => {
  ...
  const getDetails = async () => {
    try {
      const { data } = await axios.get(`/api/v1/complaint/bike-details/${id}`);
      if (data?.success) {
        setComplaint(data?.bike);
      }
    } catch (error) {
      console.error(error);
      toast.error(error?.response?.data?.message || 'Failed to fetch bike details.');
    }
  };
  ...
  const toggleVerification = async () => {
    try {
      setLoading(true);
      const { data } = await axios.patch(`/api/v1/admin/update-bike-status/${id}`);
      if (data?.success) {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 mb-10 p-6 bg-white rounded-xl shadow-lg space-y-4 border">
      <h2 className="text-2xl font-semibold text-center mb-4">🏍 Bike Complaint Details</h2>
